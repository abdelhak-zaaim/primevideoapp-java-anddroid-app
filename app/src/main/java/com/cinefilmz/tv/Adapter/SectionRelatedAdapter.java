...
public class SectionRelatedAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private String videoType, typeID, layoutType = "";
    private List<GetRelatedVideo> relatedVideoList;

    public SectionRelatedAdapter(Activity context, List<GetRelatedVideo> relatedVideoList, String videoType, String typeID) {
    ...
    **************************** */
    ...
    public void onBindVideoHolder(VideoHolder holder, int position) {
        ...
        if (layoutType.equalsIgnoreCase("square")) {
            if (!TextUtils.isEmpty(relatedVideoList.get(position).getThumbnail())) {
                Picasso.get().load(relatedVideoList.get(position).getThumbnail()).placeholder(R.drawable.no_image_port).into(holder.ivThumb);
            } else {
            ...
            if (!TextUtils.isEmpty(relatedVideoList.get(position).getLandscape())) {
                Picasso.get().load(relatedVideoList.get(position).getLandscape()).placeholder(R.drawable.no_image_land).into(holder.ivThumb);
            } else {
                Picasso.get().load(R.drawable.no_image_land).placeholder(R.drawable.no_image_land).into(holder.ivThumb);
            }
        }
        if (!TextUtils.isEmpty(relatedVideoList.get(position).getReleaseTag())) {
            holder.lyNewTag.setVisibility(View.VISIBLE);
            holder.txtNewTag.setText("" + relatedVideoList.get(position).getReleaseTag());
        } else {
        ...
        holder.lyContent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Log.e("itemID", "==>> " + relatedVideoList.get(position).getId());
                Log.e("videoType", "==>> " + videoType);
                Log.e("typeID", "==>> " + typeID);
                Utility.openDetails(context, "" + relatedVideoList.get(position).getId(), "" + videoType, "" + typeID,
                        "" + relatedVideoList.get(position).getUpcomingType());
            }
        });

        holder.lyContent.setOnLongClickListener(v -> {
            Utility.showLongPressedMovieDialog(context, new ArrayList<>(), new ArrayList<>(), relatedVideoList, new ArrayList<>(), position);
            return false;
        });
    }
    /* **** Video ***** */
    ...
    public void onBindShowHolder(ShowHolder holder, int position) {
        ...
        holder.lyContent.setOnLongClickListener(v -> {
            Utility.showLongPressedTVShowDialog(context, new ArrayList<>(), new ArrayList<>(), relatedVideoList, new ArrayList<>(), position);
            return false;
        });
